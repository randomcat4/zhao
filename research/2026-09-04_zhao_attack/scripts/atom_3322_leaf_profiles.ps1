param(
    [ValidateSet('3322')][string]$Mode = '3322',
    [int]$Cutoff = 13,
    [int]$Target = 16,
    [string]$OutputFile = '',
    [string]$SkipEvidence = '',
...
using System.IO;
using System.Text;
public static class Atom3322LeafProfiles {
    ...
    static void Profile(byte[] d,int depth){
        ...
        evidenceWriter.WriteLine("{\"type\":\"leaf_profile\",\"root\":"+path[0]+",\"blocks\":"+Ints(blocks)+",\"length\":"+(coreLength+2*depth)+",\"coverage\":"+(Q-hist[INF])+",\"unreachable_count\":"+hist[INF]+",\"maximum_distance\":"+maximum+",\"finite_maximum_distance\":"+finiteMaximum+",\"distance_counts\":"+counts+",\"safe_single_additions\":"+Ints(safe.ToArray())+",\"safe_single_additions_cutoff14\":"+Ints(safe14.ToArray())+"}");
    }
    ...
    public static void Run(string mode,int cutoff,int target,string filename,int[] skip,int seconds,long limit,string sha){
        coreLength=10;M=cutoff;BLOCKS=(target-coreLength)/2;
        ...
        for(int g=1;g<Q;g++)if(!basisSet.Contains(g)&&Safe(d,g)){initial.Add(g);bool canonical=digits[g,0]>=digits[g,1]&&digits[g,2]>=digits[g,3];if(canonical)roots.Add(g);}
...
[Atom3322LeafProfiles]::Run($Mode,$Cutoff,$Target,$OutputFile,[int[]]$atomSkip,$Seconds,$NodeLimit,$atomSha)
