...
using System.IO;
using System.Text;
public static class AtomBSupport9 {
    const int Q=625, INF=99, M=14, BLOCKS=5;
    static int[] plus=new int[Q*Q],neg=new int[Q],twice=new int[Q],path=new int[BLOCKS],longest,firstTerminal;
    static int[,] digits=new int[Q,4];
    static long[] levels=new long[BLOCKS+1],leaves=new long[BLOCKS+1];
    ...
    static void Dfs(byte[] d,int[] cand,int count,int depth){
        ...
        if(depth==BLOCKS){terminals++;if(firstTerminal==null){firstTerminal=new int[depth];Array.Copy(path,firstTerminal,depth);}return;}
        bool child=false;
    ...
    static string Levels(long[] a){var s=new StringBuilder("{");bool f=true;for(int i=0;i<a.Length;i++)if(a[i]>0){if(!f)s.Append(',');s.Append('"').Append(10+2*i).Append("\":").Append(a[i]);f=false;}return s.Append('}').ToString();}
    static void Line(StreamWriter w,string s){w.WriteLine(s);w.Flush();Console.WriteLine(s);}
    public static void Run(string filename,int[] skip,int seconds,long limit,string sha){
        ...
        int[] basis={1,5,25,125};for(int j=0;j<4;j++){basisSet.Add(basis[j]);for(int k=0;k<(j<2?3:2);k++){if(d[neg[basis[j]]]!=INF)throw new Exception("core zero sum");d=Adjoin(d,basis[j]);}}
        var initial=new List<int>();var roots=new List<int>();
        for(int g=1;g<Q;g++)if(!basisSet.Contains(g)&&Safe(d,g)){initial.Add(g);if(digits[g,0]>=digits[g,1]&&digits[g,2]>=digits[g,3])roots.Add(g);}
        var skipSet=new HashSet<int>(skip);watch=Stopwatch.StartNew();int attempted=0,completed=0,maximum=10;long totalNodes=0,totalTerminal=0;
        using(var w=new StreamWriter(filename,false,new UTF8Encoding(false))){
            Line(w,"{\"type\":\"metadata\",\"group\":\"C_5^4\",\"core_multiplicities\":[3,3,2,2],\"m\":14,\"target\":20,\"new_blocks\":5,\"block_multiplicity\":2,\"initial_safe_count\":"+initial.Count+",\"initial_safe_candidates\":"+Ints(initial.ToArray())+",\"canonical_roots\":"+Ints(roots.ToArray())+",\"script_sha256\":\""+sha+"\"}");
...
[AtomBSupport9]::Run($OutputFile,[int[]]$atomSkip,$Seconds,$NodeLimit,$atomSha)
