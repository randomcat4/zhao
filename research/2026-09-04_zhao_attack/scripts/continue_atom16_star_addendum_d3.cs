...
public static class ContinueAtom16StarAddendumD3 {
    ...
    static object RunCore(string name,int[] values,int[] caps){
        Array.Clear(nodes,0,nodes.Length);leafSumTests=0;timedOut=false;found=false;witness=null;
        Array.Clear(forbidden,0,forbidden.Length);foreach(int g in values)forbidden[g]=true;
        ...
        return new {name,complete=!timedOut,timed_out=timedOut,found_safe_B20_witness=found,
            target_W_sum=targetSum,nodes_by_W_length=nodes,leaf_sum_tests=leafSumTests,witness_W=witness,seconds=timer.Elapsed.TotalSeconds};
    }
    ...
    public static void Run(string root,double limit){
        ...
        var first=RunCore("k=(2,1,1), m on the k=2 value",values,new[]{1,3,3,1,2,1,2});
        var second=RunCore("k=(2,1,1), m on a k=1 value",values,new[]{1,2,3,2,2,1,2});
        string[] inputs={"proofs/continue_atom16_star.md","scripts/continue_atom16_star_addendum_d3.cs","scripts/continue_atom16_star_addendum_d3.ps1"};
        var hashes=inputs.ToDictionary(x=>x,x=>Hash(Path.Combine(root,x)));
        var output=new {status="TARGETED_D3_M1_EXHAUSTIVE_EXTENSION_TEST",scope="Two forced normalized 13-position cores; W has seven positions, all distinct or one double",cores=new[]{first,second},input_sha256=hashes};
        File.WriteAllText(Path.Combine(root,"evidence/continue_atom16_star_addendum_d3.json"),JsonSerializer.Serialize(output,new JsonSerializerOptions{WriteIndented=true})+"\n",new UTF8Encoding(false));
        Console.WriteLine(JsonSerializer.Serialize(new{output.status,cores=output.cores}));
    }
}
