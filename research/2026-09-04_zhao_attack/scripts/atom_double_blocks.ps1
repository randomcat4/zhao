param(
    [ValidateSet('3322','2222')][string]$Mode = '3322',
    [int]$Cutoff = 13,
    [int]$Target = 20,
    [string]$OutputFile = '',
    [string]$SkipEvidence = '',
...
using System.IO;
using System.Text;
public static class AtomDoubleBlocks {
    ...
    public static void Run(string mode,int cutoff,int target,string filename,int[] skip,int seconds,long limit,string sha){
        bool allDouble=mode=="2222";coreLength=allDouble?8:10;M=cutoff;BLOCKS=(target-coreLength)/2;
        ...
        int[] basis={1,5,25,125};for(int j=0;j<4;j++){basisSet.Add(basis[j]);for(int k=0;k<(!allDouble&&j<2?3:2);k++){if(d[neg[basis[j]]]!=INF)throw new Exception("core zero sum");d=Adjoin(d,basis[j]);}}
        var initial=new List<int>();var roots=new List<int>();
        for(int g=1;g<Q;g++)if(!basisSet.Contains(g)&&Safe(d,g)){initial.Add(g);bool canonical=digits[g,0]>=digits[g,1]&&digits[g,2]>=digits[g,3]&&(!allDouble||digits[g,1]>=digits[g,2]);if(canonical)roots.Add(g);}
...
[AtomDoubleBlocks]::Run($Mode,$Cutoff,$Target,$OutputFile,[int[]]$atomSkip,$Seconds,$NodeLimit,$atomSha)
