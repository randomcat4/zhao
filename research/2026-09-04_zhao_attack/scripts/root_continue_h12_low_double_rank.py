"""Finite bounded classification probe for H12 height2 with double-value rank1/2."""
...
def run(rank):
 base=[g for j,g in enumerate([1,5,25]) for _ in range(2 if j<rank else 1)]
 bits=1
 for g in base:bits|=shift(bits,g)
 caps=[2 if not any(vec[g][rank:]) else 1 for g in range(125)]
 pool=[g for g in range(1,125) if g not in [1,5,25] and not (bits>>neg[g])&1]
 perms=[p for p in permutations(range(3)) if set(p[:rank])==set(range(rank))]
 ...
 try:
  for g in roots:
   ...
   candidates=[x for x in pool if x>=g and not (nxt>>neg[x])&1 and (x!=g or caps[g]>1)]
   before=nodes;dfs(base+[g],nxt,candidates);complete.append({'root':g,'nodes':nodes-before})
 except Stop:status='INCOMPLETE_TIMEOUT'
 out={'scope':'height2 H12; rank of doubled actual values exactly '+str(rank),'status':status,'base':base,'initial_pool':len(pool),'roots':roots,'completed_roots':complete,'nodes':nodes,'leaves':leaves,'depth_counts':dict(depths),'max_distance_except_total_histogram':dict(hist),'E9_histogram':dict(Counter(r['E9'] for r in profiles)),'E10_histogram':dict(Counter(r['E10'] for r in profiles)),'seconds':time.monotonic()-start}
 (root/f'evidence/root_continue_h12_double_rank{rank}.json').write_text(json.dumps(out,indent=2),encoding='utf8')
 (root/f'evidence/root_continue_h12_double_rank{rank}_profiles.jsonl').write_text('\n'.join(json.dumps(r) for r in profiles)+'\n',encoding='utf8')
 print(json.dumps(out),flush=True)
for rank in [2,1]:run(rank)
