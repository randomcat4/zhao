...
## LEMMA-P233-THREE-H8-M4-O1-DELETION-QUINTIC：删点奇五次闭合

- 精确陈述：
  `proofs/unique_tail_p233_three_h8_m4_O1_single_deletion_quintic_exclusion.md`。
- 状态：PROVED_SUBBRANCH；两份独立十二维有限域实现审计 CORRECT；
  GLOBAL_INCOMPLETE。
- 若 \(g\ne0\) 是共同核中一个真实位置的标签，删去该位置后的系数
  函数 \(d\) 总次数至多五，并在中心 \(-\tau-g/2\) 关于补集反射为
  奇函数。故其中心奇五次 \(D_g\) 满足
  \[
  D_g(s+g/2)-D_g(s-g/2)=q(s).
  \]
  固定 \(g\) 后通解核恰为
  \(\langle Y,Y^3,Y^5\rangle\)，没有漏掉其他删点生成积。
- 对三个 fringe 的每个非空字面子集 \(A\)，原子性先给
  \(D\) 中不存在和值 \(-\sigma(A)\) 的普通表示，再给
  \(d(-\sigma(A))=0\)；即使 \(A=F_i\) 也因删点而仍是真子序列。
- 四例外各有 45 条字面行、23 个不同目标。对全部
