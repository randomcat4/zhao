...
int mind[125];long long roots=0,sevens=0;int maxcap=0,maxsame=0;long long hist[626]={};
void verify_seven(const array<int,7>&R){
 ...
 for(int q=0;q<5;q++)for(int u=0;u<125;u++){
  ...
  if(!safe)continue;
  if(q==1)same++;
  if(q==0){if(u==1||u==5||u==25||u==0)continue;cap+=2;}
  else {bool existing=q==1&&find(R.begin(),R.end(),u)!=R.end();if(!existing)cap++;}
 }
 hist[cap]++;maxcap=max(maxcap,cap);maxsame=max(maxsame,same);
 if(cap>4||same){cerr<<"DISAGREEMENT cap="<<cap<<" same="<<same<<" R";for(int v:R)cerr<<" "<<v;cerr<<endl;exit(2);}
}
int main(){
 ...
 cout<<"INDEPENDENT COMPLETE roots="<<roots<<" normalized_sevens="<<sevens<<" max_extension_capacity="<<maxcap<<" max_same_coset_extensions="<<maxsame<<" cap_hist=";
