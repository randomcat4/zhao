...
Mask allow[10][8][125],ok[10][5]; int M=5,NEED=6,coreg=26;vector<int> Hbase; int qchosen[5]={};
...
void dfs(BS P,int hused){nodes++;if(chosen.size()==NEED){found=true;cout<<"FOUND_A";for(int x:core)cout<<" "<<x;for(int i:chosen)cout<<" "<<verts[i];cout<<endl;return;}
 ...
 for(int i=(int)ord.size()-1;i>=0;i--){if(chosen.size()+bounds[i]<NEED)return;int v=ord[i];if(!P[v])continue;int g=verts[v];
  bool safe=!(g<125&&hused>=1) && !(g>=125&&qchosen[g/125]>=M);for(auto [s,k]:subsetlist)if(safe&&!allowed(k+1,add(s,g)))safe=false;
...
void process(const vector<int>&R){
 cores++;for(int coordinate=0,p=1;coordinate<3;coordinate++,p*=5){bool eq=true;for(int r:R)if((r/p)%5!=(R[0]/p)%5){eq=false;break;}if(false){dense++;return;}}
 ...
 for(int u=1;u<125;u++)if(u!=1&&u!=5&&u!=25&&u!=coreg&&ok[1][0].has(u))hc.push_back(u);
 int hcap=hc.empty()?0:1;
 ...
 for(int u:hc){verts.push_back(u);}n=verts.size();if(n>512){cerr<<"overflow"<<endl;exit(3);}graphv.assign(n,BS());for(int i=0;i<n;i++)for(int j=i+1;j<n;j++)if(allowed(2,add(verts[i],verts[j]))){graphv[i].set(j);graphv[j].set(i);}
 P.reset();for(int i=0;i<n;i++)P.set(i);o.clear();b.clear();color(P,o,b);if(b.empty()||b.back()<NEED)return;colorpass++;
 core=Hbase;for(int r:R)core.push_back(125+r);chosen.clear();subsetlist={{0,0}};dfs(P,0);
}
int main(int argc,char**argv){int first=argc>1?atoi(argv[1]):1,last=argc>2?atoi(argv[2]):124;M=argc>3?atoi(argv[3]):5;NEED=11-M;coreg=argc>4?atoi(argv[4]):26;if(M<3||M>6)return 3;
 ...
 Hbase={1,1,1,5,5,5,25,25,coreg,coreg}; array<int,125> hd;hd.fill(99);hd[0]=0;
 for(int g:Hbase){if(hd[ng[g]]<13)return 4;auto before=hd;for(int u=0;u<125;u++)hd[ad[u][g]]=min(hd[ad[u][g]],before[u]+1);}for(int u=0;u<125;u++)dh[u]=hd[u];
 ...
 cout<<"COMPLETE H10_g="<<coreg<<" class="<<M<<" interval="<<first<<":"<<last<<" roots="<<roots<<" cores="<<cores<<" dense="<<dense<<" capacity_pass="<<capacitypass<<" color_pass="<<colorpass<<" nodes="<<nodes<<" max_capacity="<<maximum_capacity<<" A_endpoints=0"<<endl;
}
