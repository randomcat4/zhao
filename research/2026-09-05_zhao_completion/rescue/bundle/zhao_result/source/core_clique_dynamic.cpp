...
void color(BS P,vector<int>&order,vector<int>&bound,const Dist*dist=nullptr){int co=0;while(P.any()){co++;BS Q=P;while(Q.any()){int v=Q._Find_first();P.reset(v);Q.reset(v);if(dist){for(int u=Q._Find_first();u<625;u=Q._Find_next(u))if(edges[v][u]&&(*dist)[ng[addg[vals[v]][vals[u]]]]>=12)Q.reset(u);}else Q&=~edges[v];order.push_back(v);bound.push_back(co);}}}
void dfs(BS P,const Dist&d){nodes++;if((int)path.size()==need){cout<<"FOUND";for(int x:core)cout<<" "<<x;for(int x:path)cout<<" "<<x;cout<<endl;found=true;return;}
 ...
 if(path.size()+P.count()<need)return;vector<int>ord,b;color(P,ord,b,&d);
