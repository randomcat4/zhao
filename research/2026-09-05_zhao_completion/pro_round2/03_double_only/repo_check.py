#!/usr/bin/env python3
"""Repository-copy finite-interface checker for the double-only Zhao proof.
...
def main() -> None:
    ...
    parser.add_argument("proof_dir", type=pathlib.Path)
    parser.add_argument("--json", type=pathlib.Path, default=pathlib.Path("repo_check.json"))
    args = parser.parse_args()

    base = args.proof_dir.resolve()
    require(base.is_dir(), "proof directory missing")
    manifest = base / "SHA256SUMS.txt"
    require(manifest.is_file(), "proof manifest missing")
    result: dict[str, object] = {
        "scope": "repository-copy integrity and finite interfaces; universal proof audited separately",
        "proof_dir": str(base),
    }
    ...
    print("REPOSITORY_FINITE_INTERFACE_CHECK_PASS")
    print(f"proof_dir={base}")
    print(f"manifest_files={len(result['manifest_files'])}; deleted_position_records={records}; n15_branches={len(branch_rows)}")
    print("This replays the independently written finite-interface logic against the committed proof directory and is not a global sequence enumeration.")
